...
func Execute(ctx context.Context, version string) {
	...
	rootCmd.Flags().BoolVarP(&input.noPull, "no-pull", "", false, "pull docker image(s) even if already present")
	rootCmd.Flags().BoolVarP(&input.noRebuild, "no-rebuild", "", false, "rebuild local action docker image(s) even if already present")
