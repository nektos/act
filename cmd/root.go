...
func createRootCommand(ctx context.Context, input *Input, version string) *cobra.Command {
	...
	rootCmd.PersistentFlags().StringVarP(&input.artifactServerAddr, "artifact-server-addr", "", "localhost", "Defines the address to which the artifact server binds.")
	...
	rootCmd.PersistentFlags().StringVarP(&input.cacheServerAddr, "cache-server-addr", "", "localhost", "Defines the address to which the cache server binds.")
