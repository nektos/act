...
func createRootCommand(ctx context.Context, input *Input, version string) *cobra.Command {
	...
	rootCmd.Flags().BoolVarP(&input.allowConcurrentRuns, "parallel", "", false, "take measures to permit multiple instances of act to run simultaneously against the same Docker server, but has no effect if --reuse is present (without this setting, strange conflicts and failures can happen)")
