...
func createRootCommand(ctx context.Context, input *Input, version string) *cobra.Command {
	...
	rootCmd.PersistentFlags().StringVarP(&input.artifactServerAddr, "artifact-server-addr", "", "", "Defines the address to which the artifact server binds. If not specified, the outbound IP is auto-detected.")
	...
	rootCmd.PersistentFlags().StringVarP(&input.cacheServerAddr, "cache-server-addr", "", "", "Defines the address to which the cache server binds. If not specified, the outbound IP is auto-detected.")
...
func newRunCommand(ctx context.Context, input *Input) func(*cobra.Command, []string) error {
	return func(cmd *cobra.Command, args []string) error {
		...
		// Auto-detect outbound IP for artifact and cache servers if not specified
		if input.artifactServerAddr == "" || input.cacheServerAddr == "" {
			ip := common.GetOutboundIP()
			if ip == nil {
				return fmt.Errorf("unable to determine outbound IP address for artifact/cache server")
			}
			if input.artifactServerAddr == "" {
				input.artifactServerAddr = ip.String()
				log.Debugf("Auto-detected outbound IP for artifact server: %s", input.artifactServerAddr)
			}
			if input.cacheServerAddr == "" {
				input.cacheServerAddr = ip.String()
				log.Debugf("Auto-detected outbound IP for cache server: %s", input.cacheServerAddr)
			}
		}
