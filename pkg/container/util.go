...
func GetSocketAndHost(containerSocket string, dockerHostEnv string) (string, string, error) {
	log.Debugf("Handling container host and socket")

	// Prefer DOCKER_HOST, don't override it
	dockerHost, hasDockerHost := os.LookupEnv(dockerHostEnv)

	// ** containerSocket cases **
	...
	// A - (dash) in containerSocket means don't mount, preserve this value
	// otherwise if containerSocket is a filepath don't use it as socket
	// Exit early if we're in an invalid state (e.g. when no DOCKER_HOST and user supplied "-", a dash or omitted)
	if !hasDockerHost && containerSocket != "" && !isDockerHostURI(containerSocket) {
		// Cases: 1B, 2B
		// Should we early-exit here, since there is no host nor socket to talk to?
		return "", "", fmt.Errorf("daemon Docker Engine socket not found and containerSocket option was not set")
	}

	// Default to DOCKER_HOST if set
	if containerSocket == "" && hasDockerHost {
		// Cases: 4A
		log.Debugf("Defaulting container socket to DOCKER_HOST")
		containerSocket = dockerHost
	}
	...
	if hasDockerHost {
		// Cases: 1A, 2A, 3A, 4A
		if !isDockerHostURI(containerSocket) {
			// Cases: 1A, 2A
			log.Warnf("DOCKER_HOST is set, but socket is invalid '%s'", containerSocket)
		}
		return "", "", nil
	}
	...
	if isDockerHostURI(containerSocket) {
		...
		os.Setenv("DOCKER_HOST", containerSocket)
	...
	return "", "", fmt.Errorf("no DOCKER_HOST and an invalid container socket '%s'", containerSocket)
}
