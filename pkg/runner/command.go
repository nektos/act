...
func (rc *RunContext) setEnv(ctx context.Context, kvPairs map[string]string, arg string) {
	logger.Logger(ctx).Infof("  \U00002699  ::set-env:: %s=%s", kvPairs["name"], arg)
	if rc.Env == nil {
...
func (rc *RunContext) setOutput(ctx context.Context, kvPairs map[string]string, arg string) {
	...
	if !ok {
		logger.Logger(ctx).WithField("emoji", "  \U00002757").Infof("  no outputs used step '%s'", stepID)
		return
	}

	logger.Logger(ctx).WithField("emoji", "  \U00002699").Infof("  ::set-output:: %s=%s", outputName, arg)
	result.Outputs[outputName] = arg
}

func (rc *RunContext) addPath(ctx context.Context, arg string) {
	logger.Logger(ctx).WithField("emoji", "  \U00002699").Infof("  ::add-path:: %s", arg)
