...
func (rc *RunContext) getGithubContext() *githubContext {
	...
	case "pull_request":
		ghc.BaseRef = ghc.Event.PullRequest.Base.Ref
		ghc.HeadRef = ghc.Event.PullRequest.Head.Ref
	default:
		if ghc.Event.Ref != "" {
			log.Debugf("using github ref from event: %s", ghc.Event.Ref)
			ghc.Ref = ghc.Event.Ref
		} else {
			...
			if ghc.Event.Repository.DefaultBranch == "" {
				// set the branch in the event data
				if rc.Config.DefaultBranch != "" {
					ghc.Event.Repository.DefaultBranch = rc.Config.DefaultBranch
				} else {
					ghc.Event.Repository.DefaultBranch = "master"
				}
			}
		}
	}
